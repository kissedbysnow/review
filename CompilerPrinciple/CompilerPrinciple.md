...
- **替换规则**
  ...
  <img src="images\3.3.4 替换规则.png&nbsp;" alt="3.3.4 替换规则" style="zoom:60%;" />
...
- **例：**
  ...
  > <img src="images\3.3.4-确定化过程-例题.png&nbsp;" alt="3.3.4-例题" style="zoom:60%;" />
  >
  > | I           | Ia                                                           | Ib                                                           |
  > | ----------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
- **最小化的具体做法：**
  ...
  - 既定到某个时候，π 已含 m 个子集，记为 π={I¹,I²,...}，检查 π 中的每个子集看能否进一步划分：
    ...
    对某个 I^i，令 I^i={s₁,s₂,...} ，若存在一个输入字符 a 使得 Ia^i 不会包含在现行 π 的某个子集 I^i 中，则至少应把 I^i 分为两个部分

- 例

  > ![3.3.5-例题-1](images\3.3.5-例题-1.png)
  >
  > <img src="images\3.3.5-例题-2.png&nbsp;" alt="3.3.5-例题-2" style="zoom:60%;" />